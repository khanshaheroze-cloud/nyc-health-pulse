...
export default function EatSmartMapHeroImpl({ center, restaurants, selectedId, onMapMove, onPinClick, flyToRef, distanceUnit = "blocks" }: Props) {
  ...
  useEffect(() => {
    ...
    restaurants.forEach((r) => {
      ...
      // Drink-first bars: findable, dimmed, never ranked (round 5)
      if (r.drinkFirst) el.style.opacity = "0.45";
      ...
      const popupHtml = `
        <div style="min-width:200px;max-width:280px;font-family:system-ui,-apple-system,sans-serif;">
          ...
          ${r.drinkFirst ? `<p style="font-size:10px;font-weight:700;color:#8A6A1C;background:#FBF6E8;border:1px solid #F0E3B5;border-radius:6px;display:inline-block;padding:1px 6px;margin:0 0 3px;">Bar — not in ranked picks</p>` : ""}
