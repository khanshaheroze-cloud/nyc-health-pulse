...
import { pm25Neighborhoods as STATIC_PM25_NBHD, pollutantsByBorough as STATIC_POLLUTANTS, pm25Trend, COLORS } from "@/lib/data";
...
export function Pm25NeighborhoodChart({ data }: { data?: Pm25Row[] }) {
  const chartData = [...(data ?? STATIC_PM25_NBHD)].reverse();
  const tag = data ? "LIVE" : "2023";
  return (
    <ChartCard
      title="PM2.5 by Neighborhood"
      subtitle="Annual μg/m³ · NYCCAS · WHO guideline = 5"
      fullWidth
      tall
      tag={tag}
      whyItMatters="PM2.5 is fine particulate matter that penetrates deep into your lungs. The WHO recommends below 5 μg/m³ — every NYC neighborhood exceeds that guideline."
    >
...
export function PollutantsByBoroughChart({ data }: { data?: PollutantRow[] }) {
  const tag = data ? "LIVE" : "2023";
  return (
    <ChartCard
      title="Air Pollutants by Borough"
      subtitle="PM2.5 (μg/m³) · NO₂ (ppb) · O₃ (ppb) · NYCCAS"
      tag={tag}
      whyItMatters="NO₂ comes primarily from vehicle exhaust and building boilers. Long-term exposure worsens asthma and increases cardiovascular risk."
    >
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data ?? STATIC_POLLUTANTS} barGap={2}>
          ...
          <Bar dataKey="o3"   name="O₃"    unit=" ppb"   fill={COLORS.blue  + "cc"} radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </ChartCard>
  );
