...
export interface ItemSource {
  ...
  externalId?: string;
  /** USDA-composed items: array of component FDC IDs + gram weights */
...
export interface RestaurantMenu {
  ...
  /** True when a chain's healthiest item still scores below 75 — high-calorie chains */
...
/** Detect meal slot from current NYC hour */
export function detectMealSlot(): "breakfast" | "lunch" | "dinner" | "snack" {
  ...
  return "snack"; // 9pm+ or before 6am
}

/** Detect which meal tab to auto-select */
export function detectMealTab(): "lunch" | "dinner" | "breakfast" | "coffee" {
  const nycHour = parseInt(
    new Intl.DateTimeFormat("en-US", {
      hour: "numeric",
      hour12: false,
  ...
  if (nycHour >= 11 && nycHour < 15) return "lunch";
  if (nycHour >= 15 && nycHour < 17) return "coffee"; // afternoon break
  if (nycHour >= 17 && nycHour < 21) return "dinner";
  return "dinner"; // 9pm+ or before 6am
}
